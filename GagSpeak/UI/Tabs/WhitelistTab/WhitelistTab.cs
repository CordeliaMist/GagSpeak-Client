    ...
    private void DrawWhitelist() {
        ...
        using (var table = ImRaii.Table("WhitelistTable", 2)) {
            ...
            using (var table2 = ImRaii.Table("RelationsManagerTable", 2, ImGuiTableFlags.RowBg)) {
                ...
                ImGui.Text($"{whitelist[_currentWhitelistItem]._yourStatusToThem}");
                ...
                ImGui.Text($"{whitelist[_currentWhitelistItem]._theirStatusToYou}");
            ...
            ImGui.SameLine();
            if(whitelist[_currentWhitelistItem]._yourStatusToThem == "None") {
                ImGui.BeginDisabled();
        ...
        // 
        if (whitelist[_currentWhitelistItem]._pendingRelationRequestFromPlayer == "Mistress" || 
            whitelist[_currentWhitelistItem]._pendingRelationRequestFromPlayer == "Pet" ||
            whitelist[_currentWhitelistItem]._pendingRelationRequestFromPlayer == "Slave") {
            // Display buttons only if there is an incoming request
            var relationText = whitelist[_currentWhitelistItem]._pendingRelationRequestFromPlayer?.Split(' ')[0];
            if (ImGui.Button($"Accept {whitelist[_currentWhitelistItem].name.Split(' ')[0]} as your {relationText}", new Vector2(ImGui.GetContentRegionAvail().X/2, 25))) {
                ...
                if (whitelist[_currentWhitelistItem]._pendingRelationRequestFromPlayer == "Mistress") {
                    ...
                    _config, _chatManager, _gagMessages, _clientState, _chatGui);
                } else if (whitelist[_currentWhitelistItem]._pendingRelationRequestFromPlayer == "Pet") {
                    GagButtonHelpers.AcceptPetRequestFromPlayer(_currentWhitelistItem, _config.Whitelist[_currentWhitelistItem], 
                    _config, _chatManager, _gagMessages, _clientState, _chatGui);
                } else if (whitelist[_currentWhitelistItem]._pendingRelationRequestFromPlayer == "Slave") {
                ...
                whitelist[_currentWhitelistItem]._pendingRelationRequestFromPlayer = "Established";
            ...
            if (ImGui.Button($"Decline {whitelist[_currentWhitelistItem].name.Split(' ')[0]}'s Request", new Vector2(ImGui.GetContentRegionAvail().X, 25))) {
                ...
                whitelist[_currentWhitelistItem]._pendingRelationRequestFromPlayer = "";
