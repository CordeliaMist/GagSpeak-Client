...
public partial class WhitelistPanel {
    ...
    private void DrawShelfSelection() {
        ...
        $"View {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]}'s Basic Attributes & Settings", _activePanelTab == WhitelistPanelTab.Overview)) {
            _activePanelTab = WhitelistPanelTab.Overview;
        }
        ImGui.SameLine();
        if (ImGuiUtil.DrawDisabledButton($"{AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]}'s Settings",
        buttonSize2,
        $"Inspect what settings {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]} has enabled for you.\n"+
        ...
        ImGui.SameLine();
        if (ImGuiUtil.DrawDisabledButton($"Your Settings for {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]}",
        buttonSize3,
        $"Configure which permissions you want to give {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]} Access to.\n"+
        $"Any Settings you enable here, {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]} will be able to use.\n"+
        $"{AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]} can override your Settings if they have a strong enough Dynamic Tier.\n"+
        "Look in the Overview Section of the Whitelist Tab to see how people aquire certain Tiers",
    ...
    private void DrawPlayerPermissionsButtons() {
        ...
        if(ImGui.IsItemHovered()) {
            ImGui.SetTooltip($"View a fucking adorable mini-profile window.\nThis window displays all information about {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]}'s active gags.");
        }
        ...
        if(_characterHandler.whitelistChars[_characterHandler.activeListIdx]._yourStatusToThem == RoleLean.None) {
            ...
            try
            {
                ...
                if(ImGui.IsItemHovered()) {
                    ImGui.SetTooltip($"Removes both ends of the dynamic relation with {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]}.");
                }
            }
            finally {
                ImGui.EndDisabled();
            ...
            $"Removes both ends of the dynamic relation with {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]}.\n\nMust hold CTRL and SHIFT to execute.",
        ...
        if (ImGui.Button("Request Info", buttonWidth)) {
            ...
            if (_config.sendInfoName == "" && _config.acceptingInfoRequests == true && _config.processingInfoRequest == false)
            {
                ...
                _config.SetSendInfoName(AltCharHelpers.FetchNameWorldFormatByTupleIdx(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess));
        ...
        if(ImGui.IsItemHovered()) {
            ImGui.SetTooltip($"Sends a request for information to {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess)}.\n"+
    ...
    public void DrawTheirSettings(ref bool _interactions) {
        ...
        try { ImGuiUtil.Center($"Inspect / Override {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]}'s Settings"); }
        finally { ImGui.PopFont(); }
        // draw the dropdowns
        var text = $"{AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]}";
        var suffix = "'s";
        try{
    ...
    public void DrawYourSettings(ref bool _interactions) {
        ...
        // draw out the collapsible tabs and their bodies
        ImGui.PushFont(_fontService.UidFont);
        try { ImGuiUtil.Center($"Set what {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]} can do to You"); }
        finally { ImGui.PopFont(); }
        // draw the warning
        if(_characterHandler.whitelistChars[_characterHandler.activeListIdx]._yourStatusToThem == RoleLean.None || _characterHandler.whitelistChars[_characterHandler.activeListIdx]._theirStatusToYou == RoleLean.None) {
            try{
                ...
                ImGuiUtil.Center($"the options you want to grant {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]} Access to.");
    ...
    private void DrawDetailsWardrobe(ref bool _interactions, string text, string suffix) {
        var tooltipText = WhitelistPanelTab.TheirSettings == _activePanelTab
            ...
            : $"Configure what wardrobe spesific permissions {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]} will be able to use on you.";
        var headerText = WhitelistPanelTab.TheirSettings == _activePanelTab
            ? $"{text}{suffix} Wardrobe Permissions & Interactions"
            : $"{text}{suffix} Wardrobe Permissions for {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]}";
    ...
    private void DrawDetailsPuppeteer(ref bool _interactions, string text, string suffix) { 
        var tooltipText = WhitelistPanelTab.TheirSettings == _activePanelTab
            ...
            : $"View what puppeteer permissions you have given {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]} access to execute with your trigger phrase.\n"+
        ...
        var headerText = WhitelistPanelTab.TheirSettings == _activePanelTab
            ...
            : $"{text}{suffix} Puppeteer Permissions for {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]}";
        if(!ImGui.CollapsingHeader($"{headerText}")) { 
            if(ImGui.IsItemHovered()) { ImGui.SetTooltip($"{tooltipText}"); }
            return;
        }
    ...
    private void DrawDetailsToybox(ref bool _interactions, string text, string suffix) { 
        var tooltipText = WhitelistPanelTab.TheirSettings == _activePanelTab
            ...
            : $"Configure what toybox permissions you wish to give {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]} access to.\n"+
        ...
        var headerText = WhitelistPanelTab.TheirSettings == _activePanelTab
            ...
            : $"{text}{suffix} Toybox Permissions for {AltCharHelpers.FetchName(_characterHandler.activeListIdx, _characterHandler.whitelistChars[_characterHandler.activeListIdx]._charNAWIdxToProcess).Split(' ')[0]}";
        if(!ImGui.CollapsingHeader($"{headerText}")) { 
            if(ImGui.IsItemHovered()) { ImGui.SetTooltip($"{tooltipText}"); }
            return;
        }
