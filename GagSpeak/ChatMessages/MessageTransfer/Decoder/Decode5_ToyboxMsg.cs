...
public partial class MessageDecoder {
    ...
    public void DecodeToyboxMsg(string recievedMessage, int encodedMsgIndex, ref List<string> decodedMessage) {
        ...
        if(encodedMsgIndex == 30) {
            ...
            string pattern = @"^\*(?<playerInfo>.+) reached into the wardrobe and unlocked the lock securing their toybox drawer
             within the wardrobe\.""Let's have some fun sweetie, mm\?""$";
        ...
        // [0] = playerMsgWasSentFrom, [1] = PlayerMesgWasSentFrom
            ...
            // define the pattern using regular expressions
            string pattern = @"^\*(?<playerInfo>.+) reached into the wardrobe and pulled out a vibrator device from the compartment,
             a smirk formed on her face while she returned to their pet\.$";
            // use regex to match the pattern
            Match match = Regex.Match(recievedMessage, pattern);
            // check if the match is sucessful
            if (match.Success) {
        ...
        // decoder for updating the intensity of the active toy with a new intensity level [ ID == 32 ]
        ...
        // decoder for executing a stored toy pattern by its name [ ID == 33 ]
        // [0] = playerMsgWasSentFrom, [1] = PlayerMesgWasSentFrom, [37] = name of the stored toy pattern
        else if(encodedMsgIndex == 33) {
            // define the pattern using regular expressions
            string pattern = @"^\*(?<playerInfo>.+) pulled out her tomestone and tappened on the (?<patternName>.+) pattern,
             which had been linked to the active vibe against their body, causing it to provide their submissive with a wonderous dose of pleasure\.$";
            // use regex to match the pattern
            Match match = Regex.Match(recievedMessage, pattern);
            // check if the match is sucessful
            if (match.Success) {
        ...
        // decoder for if the whitelisted user is toggling the lock state of the toybox UI [ ID == 34 ]
        // [0] = playerMsgWasSentFrom, [1] = PlayerMesgWasSentFrom
        else if(encodedMsgIndex == 34) {
            // define the pattern using regular expressions
            string pattern = @"^\*(?<playerInfo>.+) wrapped a layer of durable and tight concealment over the vibe, making it remain
             locked in place against their submissive's skin\.""Enjoy~""$";
            // use regex to match the pattern
            Match match = Regex.Match(recievedMessage, pattern);
            // check if the match is sucessful
            if (match.Success) {
