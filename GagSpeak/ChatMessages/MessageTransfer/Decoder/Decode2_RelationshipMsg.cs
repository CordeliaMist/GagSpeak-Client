...
public partial class MessageDecoder {
    ...
    public void DecodeRelationshipMsg(string recievedMessage, int encodedMsgIndex, ref List<string> decodedMessage) {        
        ...
        if(encodedMsgIndex == 11) {
            ...
            string pattern = @"^\*(?<playerInfo>.+) looks down upon the submissive one before them, their pleading eyes forcing a smile across their
             lips. ""I take it you would like for me to become your (?<relationType>.+)\""$";
        ...
        // [0] = commandtype, [1] = playerMsgWasSentFrom, [3] = nameOfRelationSent
            ...
            // define the pattern using regular expressions
            string pattern = @"^\*(?<playerInfo>.+) looks up at the dominant figure before them pleading eyes, apperciating their presence deeply
             and desiring to grow closer towards them\.\* ""Would you please take me in as your (?<relationType>.+)""$";
            // use regex to match the pattern
            Match match = Regex.Match(recievedMessage, pattern);
            // check if the match is successful
            if (match.Success) {
        ...
        // [0] = commandtype, [1] = playerMsgWasSentFrom
            ...
            // define the pattern using regular expressions
            string pattern = @"^\*(?<playerInfo>.+) nods in agreement with a smile\.\* ""Oh yes, most certainly\. I would love for you to
             become my (?<relationType>.+)""$";
            // use regex to match the pattern
            Match match = Regex.Match(recievedMessage, pattern);
            // check if the match is successful
            if (match.Success) {
        ...
        // [0] = commandtype, [1] = playerMsgWasSentFrom
            ...
            // define the pattern using regular expressions
            string pattern = @"^\*(?<playerInfo>.+) smiles upon hearing the request and nods in agreement as their blushed companion\. Reaching
             down to clasp a new collar snug around their submissives neck\.\* ""Yes dearest, I'd love to make you my (?<relationType>.+)""$";
            // use regex to match the pattern
            Match match = Regex.Match(recievedMessage, pattern);
            // check if the match is successful
            if (match.Success) {
        ...
        // [0] = commandtype, [1] = playerMsgWasSentFrom
            ...
            string pattern = @"^\*(?<playerInfo>.+) glanced back down at her companion who had just crawled up to their legs with the pleading
             look and smiled\.\* ""Verywell\. And I hope you're able to devote yourself to the commitment of being my (?<relationType>.+)""$";
            // use regex to match the pattern
            Match match = Regex.Match(recievedMessage, pattern);
            // check if the match is successful
            if (match.Success) {
        ...
        // [0] = commandtype, [1] = playerMsgWasSentFrom
            ...
            // define the pattern using regular expressions
            string pattern = @"^\*(?<playerInfo>.+) looks up at you with tears in her eyes\. She never wanted this moment to come, but also knows due
             to the circumstances it was enivtable\.\* ""I'm sorry, but I cant keep our relationship going right now, there is just too much going on""$";
            // use regex to match the pattern
            Match match = Regex.Match(recievedMessage, pattern);
            // check if the match is successful
            if (match.Success) {
