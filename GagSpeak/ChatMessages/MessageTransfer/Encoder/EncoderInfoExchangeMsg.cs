...
public partial class MessageEncoder {
    ...
    public string HandleProvideInfoPartOne(PlayerPayload playerPayload, string targetPlayer, CharacterHandler _characterHandler) {
        ...
        if(AltCharHelpers.IsPlayerInWhitelist(targetPlayer, out int Idx, out int CharNameIdx))
        {
            //f we reach here, it is successful, and we can begin to encode the message.
            string targetPlayerFormatted = AltCharHelpers.FetchNameWorldFormatByTupleIdx(Idx, CharNameIdx);
