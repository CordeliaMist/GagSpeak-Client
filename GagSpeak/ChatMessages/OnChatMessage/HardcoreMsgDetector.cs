...
public class HardcoreMsgDetector
{
    ...
    public bool IsValidMsgTrigger(string senderName, SeString chatmessage, XivChatType type, out SeString messageToSend) {
        ...
        if(senderName == _client.LocalPlayer!.Name.TextValue) {
            List<(string _name, string _homeworld)> names = _characterHandler.whitelistChars
                .Where(x => x._inHardcoreMode)
                .SelectMany(x => x._charNAW)
                .ToList();
            // detect if the message contains any of the hardcore commands
            foreach (var name in names) {
                // follow me
                if(chatmessage.TextValue.ToLowerInvariant().Contains($"{name._name.Split(' ')[0].ToLowerInvariant()}, follow me.")) {
                    // get the index of the name
                    if(AltCharHelpers.IsPlayerInWhitelist(name._name, out int whitelistCharIdx)) {
                        // toggle that players forcedfollow to true
                        _characterHandler.whitelistChars[whitelistCharIdx]._forcedFollow = true;
                        _characterHandler.Save();
                        return false; // return false to avoid processing anymore logic
                    }
                }
                // sit start
                if(chatmessage.TextValue.ToLowerInvariant().Contains($"{name._name.Split(' ')[0].ToLowerInvariant()}, sit.")) {
                    if(AltCharHelpers.IsPlayerInWhitelist(name._name, out int whitelistCharIdx)) {
                        _characterHandler.whitelistChars[whitelistCharIdx]._forcedSit = true;
                        _characterHandler.Save();
                        return false;
                    }
                }
                // set end
                if(chatmessage.TextValue.ToLowerInvariant().Contains($"you may stand now {name._name.Split(' ')[0].ToLowerInvariant()}.")) {
                    if(AltCharHelpers.IsPlayerInWhitelist(name._name, out int whitelistCharIdx)) {
                        _characterHandler.whitelistChars[whitelistCharIdx]._forcedSit = false;
                        _characterHandler.Save();
                        return false;
                    }
                }
                // stay here start
                if(chatmessage.TextValue.ToLowerInvariant().Contains($"{name._name.Split(' ')[0].ToLowerInvariant()}, stay here until i return.")) {
                    if(AltCharHelpers.IsPlayerInWhitelist(name._name, out int whitelistCharIdx)) {
                        _characterHandler.whitelistChars[whitelistCharIdx]._forcedToStay = true;
                        _characterHandler.Save();
                        return false;
                    }
                }
                // stay here end
                if(chatmessage.TextValue.ToLowerInvariant().Contains($"thank you for waiting, {name._name.Split(' ')[0].ToLowerInvariant()}.")) {
                    if(AltCharHelpers.IsPlayerInWhitelist(name._name, out int whitelistCharIdx)) {
                        _characterHandler.whitelistChars[whitelistCharIdx]._forcedToStay = false;
                        _characterHandler.Save();
                        return false;
                    }
                }
        ...
        // we are not processing if the message is for us from someone else.
        if(AltCharHelpers.IsPlayerInWhitelist(senderName, out int senderIdx, out int CharNameIdxOuter))
        {
